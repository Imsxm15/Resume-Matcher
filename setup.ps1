...
if ($OS_TYPE -ne "Windows") {
    ...
    Write-Host "  4. Provide an OpenAI API key via OPENAI_API_KEY" -ForegroundColor Cyan
...
if (-not $env:OPENAI_API_KEY -and -not $env:LLM_API_KEY) {
    Write-Info "Reminder: set OPENAI_API_KEY (or LLM_API_KEY/EMBEDDING_API_KEY) before starting the backend."
}
...
Next steps:
 ...
 Note: Ensure OPENAI_API_KEY is set before running the backend or Docker containers.
"@
}
