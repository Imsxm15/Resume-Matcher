...
3.  **Ejecuta la configuración**
    ...
    Esto hará lo siguiente:
      ...
      - Confirmar que las variables de entorno de OpenAI están definidas (`OPENAI_API_KEY` o `LLM_API_KEY`/`EMBEDDING_API_KEY`).
...
## 🐞 Solución de Problemas
  ...
  - **`Falta la clave de API de OpenAI`**:

      - Exporta `OPENAI_API_KEY` en tu shell o añádelo a `.env`/`.env.docker` antes de ejecutar el backend o Docker.
